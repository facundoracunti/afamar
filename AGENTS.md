...
> `tsc --noEmit` 0 errores · vitest **466/466** (39 files · +0 por esta sesión: el fix de PDF HEADER es puramente presentation/template — los 14 tests de `test_public_pdf_token.py` y los 466 de `buildPdfData.test.ts` siguen pasando sin updates) · pytest **155/155** (+0) · ESLint 0 errores nuevos (los de `DocumentPdf.tsx` `SKETCH_CONTENT_WIDTH`/`YELLOW_50`/`adicRowBreakdown`/`index` y `buildPdfData.ts:17-21` imports son baseline preexistente).
