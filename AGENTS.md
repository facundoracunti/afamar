...
> `tsc --noEmit` 0 errores · vitest **421/421** (35 files · +3 por esta sesión: 2 en `useAdditionalWorkSelection.test.ts` [round-trip conserva las claves snapshot del frente + la edición del frente las mantiene] + 1 en `buildPdfData.test.ts` [zócalo price-0 valuado por m² del material] · +3 por los tests de snapshots/hidratación: 2 en `entityFormHelpers.test.ts` + 1 en `useBudgetPieces.test.ts` · +3 por los tests de dims vacías en `useBudgetPieces.test.ts` · +7 por el guard loopback + `DEFAULT_PUBLIC_URL` + origen `VITE_API_BASE_URL` en `whatsapp.test.ts`) · pytest **127/127** (+1 por `test_measurement_comparison.py::test_price_zero_zocalo_is_valued_at_linked_material_price_m2` · +3 por `tests/test_measurement_snapshot_roundtrip.py`) · ESLint 0 errores nuevos (los 2 de `QuoteOptionsGrid.tsx:69-70` `budgetId`/`onConvertirAlternativa` son baseline preexistente documentado).
