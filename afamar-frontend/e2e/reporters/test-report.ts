...
interface TestSummary {
  ...
  project: string;
  file: string;
  /** Spec relativo a cwd, ej: 'e2e/budgets/03-budgets.spec.ts' */
  specRel: string;
  status: TestResult['status'];
  duration: number;
  errorMessage?: string;
  errorStack?: string;
  images: { name: string; bodyBase64: string }[];
}
...
interface AreaCoverage {
  ...
  hasSpec: boolean;
  specs: string[];
  state: 'missing' | 'passed' | 'failed' | 'mixed' | 'empty';
  passed: number;
  failed: number;
  skipped: number;
}
...
interface Options {
  outputFile: string;
  /** Raíz del proyecto donde vive `src/pages/` y `e2e/`. Default: cwd. */
  projectRoot?: string;
  /** Carpetas dentro de `e2e/` que son infra (no son áreas funcionales). */
  skipAreas?: string[];
}
...
function parseShotName(name: string): { area: string; sector: string } | null {
  const m = name.match(/^shot-(?:cross-)?([a-z0-9-]+?)(?:-([a-z0-9-]+))?$/);
  if (!m) return null;
  return { area: m[1] ?? 'general', sector: m[2] ?? 'screenshot' };
}

class TestReport implements Reporter {
  private outputFile: string;
  private projectRoot: string;
  private skipAreas: Set<string>;
  ...
  onTestEnd(test: TestCase, result: TestResult): void {
    const images: TestSummary['images'] = [];
  ...
  private async buildCoverage(): Promise<AreaCoverage[]> {
    ...
    const areaNames = new Set<string>(
      [...pages, ...e2eDirs].filter((n) => !this.skipAreas.has(n)),
    );
    ...
    for (const name of [...areaNames].sort()) {
      ...
      const specs = specFiles.map((f) =>
        path.relative(process.cwd(), path.join(specDir, f)).replace(/\\/g, '/'),
      );
      ...
      for (const t of this.tests) {
        if (!specs.includes(t.specRel)) continue;
        if (t.status === 'passed') passed++;
        else if (t.status === 'failed' || t.status === 'timedOut') failed++;
        else if (t.status === 'skipped' || t.status === 'interrupted') skipped++;
      }

      let state: AreaCoverage['state'];
      if (specs.length === 0) state = 'missing';
      else if (passed === 0 && failed === 0) state = 'empty';
      else if (failed > 0) state = passed > 0 ? 'mixed' : 'failed';
      else state = 'passed';
...
function render(tests: TestSummary[], coverage: AreaCoverage[], startedAt: number): string {
  ...
  interface ShotGroup {
    area: string;
    sector: string;
    shots: { project: string; bodyBase64: string }[];
  }
  const keyShots = new Map<string, ShotGroup>();
  for (const t of tests) {
  ...
  const shotsByArea = new Map<string, ShotGroup[]>();
...
<body>
  ...
  ${keyShots.size > 0
    ...
    ${[...shotsByArea.entries()]
      .map(
        ([area, groups]) => `<div class="area-block">
      <h3>${esc(area)}</h3>
      ${groups
        .map(
        ...
        ${g.shots
          .map(
            (s) =>
              `<div><span class="label-tag">${esc(s.project)}</span><a href="data:image/png;base64,${s.bodyBase64}" target="_blank"><img src="data:image/png;base64,${s.bodyBase64}" alt="${esc(g.area + '-' + g.sector)}"></a></div>`,
          )
          .join('')}
      </div>`,
        )
    ...
    ${[...byProject.entries()]
      .map(([project, ts]) => {
        ...
        <table>
          ...
          ${ts
            .map(
              (t) => `<tr class="${t.status === 'passed' ? 'pass' : t.status === 'failed' || t.status === 'timedOut' ? 'fail' : ''}">
