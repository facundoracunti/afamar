...
import styles from './BudgetFormClient.module.css';
...
export default function BudgetFormClient({
  ...
  const renderAddressPicker = (client: Client) => {
    ...
    return (
      <div className={`form-group ${s['budget-form-client__address-picker']}`}>
        <label className={s['budget-form-client__address-label']}>
          <MapPin size={14} aria-hidden="true" /> Domicilio de entrega
        </label>
        <div className={s['budget-form-client__addr-row']}>
          {showSelect ? (
            <select
              className="input"
          ...
          {!readOnly && (
            <>
              <input
                className={s['budget-form-client__addr-new-input']}
              ...
              <button
                type="button"
                className={s['budget-form-client__addr-new-btn']}
        ...
        {showSelect && (
          <small className={s['budget-form-client__address-hint']}>
