...
export default function OnlineBudgetTotals({ totalArs, totalUsd, totalConsolidated, usdRate, hayUSD }: Props) {
  return (
    <div className="card" style={{ marginBottom: 16 } as React.CSSProperties}>
      <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 24, alignItems: 'center', flexWrap: 'wrap' } as React.CSSProperties}>
        ...
        <div style={{ textAlign: 'right', background: '#dc2626', color: 'white', padding: '10px 24px', borderRadius: 8 } as React.CSSProperties}>
          ...
          {hayUSD && (<div style={{ fontSize: 14, fontWeight: 400, opacity: 0.8 } as React.CSSProperties}>{`(ARS + USD x $${Number(usdRate).toLocaleString('es-AR')})`}</div>)}
          <div style={{ fontSize: 22, fontWeight: 800 } as React.CSSProperties}>$ {totalConsolidated.toLocaleString('es-AR', { minimumFractionDigits: 2 })}</div>
        </div>
      </div>
    </div>
  );
