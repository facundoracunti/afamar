...
interface FabricacionTableProps {
  detalles: Record<string, unknown>[];
  readOnly: boolean;
  handleDetalleChange: (i: number, field: string, value: unknown) => void;
  addDetalle: () => void;
  removeDetalle: (i: number) => void;
  materiales: Record<string, unknown>[];
  CONCEPTOS_M2: string[];
  conceptosFabricacion: string[];
  num: (v: unknown) => number;
}

export default function FabricacionTable({ detalles, readOnly, handleDetalleChange, addDetalle, removeDetalle, materiales, CONCEPTOS_M2, conceptosFabricacion, num }: FabricacionTableProps) {
  return (
    <>
      ...
      <table className="table" style={{ fontSize: 13 }}>
        ...
        <tbody>
          {detalles.map((d, i) => (
            <tr key={i}>
              <td>
                <select className="input" style={{ fontSize: 12, padding: '4px 8px' }} value={d.concepto as string} onChange={(e) => handleDetalleChange(i, 'concepto', e.target.value)} disabled={readOnly}>
                  {conceptosFabricacion.map((c) => <option key={c} value={c}>{c}</option>)}
                </select>
              </td>
              <td>
                {CONCEPTOS_M2.includes(d.concepto as string) ? (
                  <select className="input" style={{ fontSize: 11, padding: '4px 4px' }} value={d.material as string || ''} onChange={(e) => handleDetalleChange(i, 'material', e.target.value)} disabled={readOnly}>
                    <option value="">--</option>
              ...
              <td>
                {CONCEPTOS_M2.includes(d.concepto as string) ? (
                  ...
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                    <input className="input" style={{ fontSize: 12, padding: '4px 8px' }} value={d.detalle as string} onChange={(e) => handleDetalleChange(i, 'detalle', e.target.value)} placeholder="DETALLES" disabled={readOnly} />
                    <div style={{ display: 'flex', gap: 4, alignItems: 'center' }}>
                  ...
                  <input className="input" style={{ fontSize: 12, padding: '4px 8px' }} value={d.detalle as string} onChange={(e) => handleDetalleChange(i, 'detalle', e.target.value)} placeholder="Cant / ML / cm" disabled={readOnly} />
                )}
              </td>
              <td>
                {CONCEPTOS_M2.includes(d.concepto as string) ? (
