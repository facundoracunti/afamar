...
interface Props {
  ...
  onCancel: () => void;
  isEdit: boolean;
  onConvertirAll: () => Promise<void>;
  saving: boolean;
}

export default function PresupuestoOnlineFooter({ onWhatsApp, onCancel, isEdit, onConvertirAll, saving }: Props) {
  return (
    <div style={{ display: 'flex', gap: 12, justifyContent: 'flex-end', flexWrap: 'wrap' } as React.CSSProperties}>
      ...
      {isEdit && (
        <button type="button" className="btn" onClick={onConvertirAll} style={{ background: '#b91c1c', color: '#fff', border: 'none', borderRadius: 8, cursor: 'pointer', fontWeight: 700, padding: '8px 20px', display: 'flex', alignItems: 'center', gap: 6 } as React.CSSProperties}>
