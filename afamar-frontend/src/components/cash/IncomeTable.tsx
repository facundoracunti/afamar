...
import { folderStatusClass } from './cashUtils';
...
export default function IncomeTable({ ingresos, cerrada, onDelete, onAdd }: Props) {
  return (
            ...
            {ingresos.length === 0 ? (
              ...
              ingresos.map((m: Record<string, unknown>) => (
                <tr key={m.id as number}>
                  ...
                  <td>
                    {m.folder_status ? (
                      <span className={`badge ${folderStatusClass(m.folder_status as string)}`}>{(m.folder_status as string)}</span>
                    ) : '-'}
                  </td>
